...
app.whenReady().then(() => {
  ...
  setTimeout(async () => {
    ...
    fetchPatchInfo(systemInfo.patches,globalUID);
  },7000);
...
async function fetchClientInfo(systemInfo,globalUID) {
  try {
    console.log('The global id inside the fetchClientInfo '+globalUID);
...
//sending the system info
async function fetchSystemInfo(systemInfo,globalUID) {
  if (!globalUID) {
    console.error('UID is not available for system info');
    return;
  }

  try {
    console.log('The global id inside the fetchClientInfo '+globalUID);
    const response = await fetch('http://10.1.32.92:5000/api/system-info', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function fetchNetworkInfo(networkInfo,globalUID) {
  ...
  try {
    console.log('The global id inside the fetchNetworkInfo '+globalUID);
    const response = await fetch('http://10.1.32.92:5000/api/network-info', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ UID:globalUID, networkData: networkInfo }),
    });
...
//sending the path info

async function fetchPatchInfo(patchesInfo,globalUID) {
  if (!globalUID) {
  ...
  try {
    const response = await fetch('http://10.1.32.92:5000/api/patch-info', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ UID:globalUID, patchData: patchesInfo }),
    });
    ...
    console.error('Error sending patch data:', error);
  }
}
...
async function sendSoftwareDataToServer(uniqueId, softwareData) {
  ...
  const serverUrl = "http://10.1.32.92:5000/api/software_info"; 
