...
app.post('/api/system-info', (req, res) => {
  ...
  const query = `INSERT INTO sys_info (UID, hostname, tmemory, fmemory, sys_release, sys_type, sys_arch) VALUES (?)`;
...
const { UID, networkData } = req.body;
  ...
  const query = `INSERT INTO network_info (UID, interface, ip_address, mac_address) VALUES ?`;
  db.query(query, [values], (err, result) => {
      if (err) {
...
app.post('/api/patch-info', (req, res) => {
    const { UID, patchData } = req.body;

    const query = `INSERT INTO patch_updates (UID, hotfix_id, description, installed_on, installed_by) VALUES ?`;
...
// sending the active process data

app.get('/process_data', (req, res) => {
  const query = `
    SELECT process_id, UID, software_data, created_at
    FROM active_process
  `;
  ...
  db.query(query, (err, results) => {
    ...
    // Format the data: Convert BLOB to string
    const formattedResults = results.map((row) => ({
      process_id: row.process_id,
      UID: row.UID,
      software_data: JSON.parse(row.software_data.toString()), // Parse JSON from BLOB
